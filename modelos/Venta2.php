...
Class Venta2
{
    ...
    $data_usuario= ejecutarConsultaSimpleFila($sql_usuario);
    if($data_usuario["incentivo_total"]>0 && $data_usuario["incentivo_total"] != ""){
      $dato_a=$data_usuario["incentivo_total"];
      $dato_n=$dato_a+$ince_total;
    }else{
      $dato_n=$ince_total;
    }
    $update_usuario="UPDATE usuario set incentivo_total=$dato_n where idusuario=$idusuario";
    ejecutarConsulta($update_usuario);

  //-- GENERAR PUNTOS POR USUARIOS--
   
  $puntos=0.00;
  $sql_persona="SELECT * FROM persona where idpersona=$idcliente";
  $data_persona= ejecutarConsultaSimpleFila($sql_persona);
  if($data_persona["puntos"]>0 && $data_persona["puntos"]!=""){
    $dato_p=$data_persona["puntos"];
    $dato_pt=round($dato_p+$total_venta,0,PHP_ROUND_HALF_UP);
 
  }else{
    $dato_pt=round($total_venta,0,PHP_ROUND_HALF_UP);
   
  }
  $update_persona="UPDATE persona set puntos=$dato_pt where idpersona=$idcliente";
  ejecutarConsulta($update_persona);
    ...
    $idventanew=ejecutarConsulta_retornarID($sql);
		...
		while ($num_elementos<count($idarticulo)) {
			...
			$sql_detalle = "INSERT INTO detalle_venta(idventa, idarticulo,cantidad,precio_venta,descuento,fecha_mas_vendido,item,serie,estado) VALUES ('$idventanew', '$idarticulo[$num_elementos]','$cantidad[$num_elementos]','$precio_venta[$num_elementos]','$descuento[$num_elementos]','$fecha_todo','$item','$serieArticulo[$num_elementos]','Aceptado')";
    ...
    =============================================*/
      ...
      =============================================*/
      $sqlLinea="SELECT dv.idventa,dv.idarticulo,a.nombre,a.unidad_medida,a.codigo,a.afectacion,dv.cantidad,dv.precio_venta,dv.descuento,dv.item,dv.serie FROM detalle_venta dv inner join articulo a on dv.idarticulo=a.idarticulo where dv.idventa='$idventanew'";
      ...
      =============================================*/
       ...
       while($rpta_linea = $rsptaDetalleVenta->fetch_object()){
              ...
              if($rpta_linea->afectacion = "Gravado"){
                ...
                // -- Descontar
                $valor_unitario = $valor_unitario -$rpta_linea->descuento;
                ...
                $precio_ventaDes=round($rpta_linea->precio_venta,2);
                $precio_ventaDes=$precio_ventaDes-$rpta_linea->descuento;
                $codTipoTributo = 1000;
                $afectacion = 10;
                $nomTributo="IGV";
                $tipoTributo="VAT";
             ...
             $data_json = '{
                "linea": [
                      ...
                      "Código_de_unidad_de_medida_por_ítem":"'.$rpta_linea->unidad_medida.'",
                       ...
                       "Precio_de_venta_unitario":"'.$precio_ventaDes.'",
	...
	public function listarDetalle($idventa)
{
    ...
    di.lote, 
    di.laboratorio, 
    DATE_FORMAT(di.fecha_vencimiento, '%d/%m/%Y') as fecha_vencimiento,
    di.precio_compra,
        a.idcategoria, 
        ...
        INNER JOIN (
            SELECT idarticulo, MAX(iddetalle_ingreso) AS iddetalle_ingreso
            FROM detalle_ingreso
            GROUP BY idarticulo
        ) di_max ON a.idarticulo = di_max.idarticulo
        INNER JOIN detalle_ingreso di ON di.iddetalle_ingreso = di_max.iddetalle_ingreso
	...
	public function ventacabecera($idventa){
		$sql="SELECT v.idventa,v.idcliente,p.nombre as cliente,p.puntos,p.direccion,p.tipo_documento,p.num_documento,p.email,p.telefono,v.idusuario,u.nombre as usuario,v.codigotipo_comprobante,v.serie,v.correlativo,date(v.fecha_hora) as fecha,v.fecha_hora as fechaCompleta,v.impuesto,v.op_gravadas,v.op_inafectas,v.op_exoneradas,v.op_gratuitas,v.isc,v.total_venta,v.total_descuentos,v.total_igv,v.idmoneda,m.descripcion as descmoneda FROM venta v INNER JOIN persona p ON v.idcliente=p.idpersona INNER JOIN usuario u ON v.idusuario=u.idusuario INNER JOIN moneda m ON m.idmoneda=v.idmoneda WHERE v.idventa='$idventa'";
		return ejecutarConsulta($sql);
	}
