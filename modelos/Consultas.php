  ...
  public function __construct()
  {
    ...
    public function ventasultimos_12meses(){
     ...
     public function kardex(){
      $sql="SELECT di.idarticulo,di.laboratorio,di.lote,a.codigo,a.nombre,a.descripcion,a.unidad_medida,c.nombre as categoria,a.stock_ingreso,a.stock_salida,a.stock FROM articulo a INNER JOIN categoria c ON a.idcategoria=c.idcategoria inner join detalle_ingreso di on 
      a.idarticulo=di.idarticulo  where a.condicion='1' ";
      return ejecutarConsulta($sql);
    }
    ...
    public function ventasfechausuario($fecha_inicio,$fecha_fin,$idusuario,$codigotipo_comprobante,$codigotipo_pago) {
      ...
      if ($idusuario == "all") {
        ...
        $sql="SELECT DATE(v.fecha_hora) as fecha,u.nombre as usuario,tp.descripcion_tipo_pago, p.nombre as cliente, p.num_documento as num_doc,v.codigotipo_comprobante,tc.descripcion_tipo_comprobante,v.serie,v.correlativo,v.total_venta,v.impuesto,v.estado from venta v inner join persona p on v.idcliente=p.idpersona inner join usuario u on v.idusuario=u.idusuario INNER JOIN tipo_comprobante tc ON tc.codigotipo_comprobante=v.codigotipo_comprobante INNER JOIN tipo_pago tp ON tp.codigotipo_pago=v.codigotipo_pago where v.idusuario='$idusuario' and date(v.fecha_hora)>='$fecha_inicio' and date(v.fecha_hora)<='$fecha_fin' $where_codigotipo_comprobante $where_codigotipo_pago 
