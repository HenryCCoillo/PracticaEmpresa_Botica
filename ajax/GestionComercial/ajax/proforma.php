...
switch ($_GET["op"]){
	case 'guardaryeditar':
		if (empty($idproforma)){
			$rspta=$proforma->insertar($idusuario,$idcliente,$correlativo,$tipo_proforma,$igv_total,$referencia,$total_venta,$fecha_hora,$_POST["descripcion"],$_POST["cantidad"],$_POST["precio"],$validez);
		...
		$rspta=$proforma->listar();
 		...
 		while ($reg=$rspta->fetch_object()){
 			...
 			$data[]=array(
				...
				"6"=>$reg->idproforma,
 				"7"=>$reg->total_venta,
 				"8"=>($reg->estado=='AceptadoP')?'<span class="label bg-green">Aceptado</span>':
