  ...
  public function __construct()
  {
    ...
    public function ventasfechausuario($fecha_inicio,$fecha_fin,$hora_inicio,$hora_fin,$idusuario, $codigotipo_comprobante,$codigotipo_pago) {
      ...
      if($hora_inicio != null AND $hora_fin != null){
        ...
        $sql="SELECT DATE_FORMAT(v.fecha_hora,'%d-%m-%Y %h:%i %p') as fecha,u.nombre as usuario, tp.descripcion_tipo_pago, p.nombre as cliente, p.num_documento as num_doc,v.codigotipo_comprobante,tc.descripcion_tipo_comprobante,v.serie,v.correlativo,v.total_venta,v.impuesto,v.estado from venta v LEFT join persona p on v.idcliente=p.idpersona LEFT join usuario u on v.idusuario=u.idusuario LEFT JOIN tipo_comprobante tc ON tc.codigotipo_comprobante=v.codigotipo_comprobante LEFT JOIN tipo_pago tp ON tp.codigotipo_pago=v.codigotipo_pago where v.idusuario='$idusuario' and date(v.fecha_hora)>='$fecha_inicio' and date(v.fecha_hora)<='$fecha_fin' $where_codigotipo_comprobante $where_codigotipo_pago and v.estado!='Cotizado' and v.estado!='Anulado' and v.estado!='AnuladoC'";
    ...
    public function sumventasfechausuario($fecha_inicio,$fecha_fin,$hora_inicio,$hora_fin,$idusuario, $codigotipo_comprobante,$codigotipo_pago) {
      ...
      // --
      if ($idusuario == "all") {
        $sql="SELECT IFNULL(sum(v.total_venta),0) AS sumatotalusuario, u.nombre as usuario FROM venta v inner join usuario u on v.idusuario=u.idusuario  where date(v.fecha_hora)>='$fecha_inicio' and date(v.fecha_hora)<='$fecha_fin' $where_codigotipo_comprobante $where_codigotipo_pago and v.estado!='Cotizado' and v.estado!='Anulado' and v.estado!='AnuladoC' ";
      } else {
        $sql="SELECT IFNULL(sum(v.total_venta),0) AS sumatotalusuario, u.nombre as usuario FROM venta v inner join usuario u on v.idusuario=u.idusuario  where v.idusuario='$idusuario' and date(v.fecha_hora)>='$fecha_inicio' and date(v.fecha_hora)<='$fecha_fin' $where_codigotipo_comprobante $where_codigotipo_pago and v.estado!='Cotizado' and v.estado!='Anulado' and v.estado!='AnuladoC' ";
      }
