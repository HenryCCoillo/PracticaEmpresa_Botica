...
Class NotaCredito{
	...
	public function insertar($idcliente,$idusuario,$serie,$correlativo,$fecha_hora,$impuesto,$op_gravadas,$op_inafectas,$op_exoneradas,$op_gratuitas,$isc,$total_igv,$total_venta,$leyenda,$idmoneda,$idtiponotacredito,$sustento,$doc_relacionado,$idarticulo,$cantidad,$precio_venta,$descuento,$tiponc3){
	   ...
	   //  }
		...
		while ($num_elementos<count($idarticulo)) {
			$sql_detalle = "INSERT INTO notacredito(idventa, idarticulo,cantidad,precio_venta,descuento,correccion_descripcion) VALUES ('$idventanew', '$idarticulo[$num_elementos]','$cantidad[$num_elementos]','$precio_venta[$num_elementos]','$descuento[$num_elementos]','$tiponc3[$num_elementos]')";
    ...
    =============================================*/
      ...
      =============================================*/
      $sqlLinea="SELECT dv.idventa,dv.idarticulo,a.nombre,a.unidad_medida,a.codigo,a.afectacion,dv.cantidad,dv.precio_venta,dv.descuento,dv.correccion_descripcion FROM notacredito dv inner join articulo a on dv.idarticulo=a.idarticulo where dv.idventa='$idventanew'";
             ...
             $data_json = '{
                "linea": [
                      ...
                      "Código_de_unidad_de_medida_por_ítem":"'.$rpta_linea->unidad_medida.'",
                       ...
                       "Descripción_detallada_del_servicio_bien_características":"'.$rpta_linea->nombre.'",
	...
	public function listarComprobantes(){
		$sql="SELECT v.idventa,v.idcliente,p.nombre as cliente,p.num_documento, v.idusuario,u.nombre as usuario,v.estado as estado,v.codigotipo_comprobante,tc.descripcion_tipo_comprobante,v.serie,v.correlativo,DATE(v.fecha_hora) as fecha,v.op_gravadas,v.op_exoneradas,v.total_venta,v.impuesto,v.idmoneda,m.descripcion FROM venta v INNER JOIN tipo_comprobante tc ON v.codigotipo_comprobante=tc.codigotipo_comprobante INNER JOIN persona p ON v.idcliente=p.idpersona INNER JOIN usuario u ON v.idusuario=u.idusuario INNER JOIN moneda m ON m.idmoneda=v.idmoneda WHERE  v.estado!='Cotizado'  and v.estado not in('AnuladoNC','Anulado') and v.codigotipo_comprobante in (1,3) ORDER BY idventa desc";
		return ejecutarConsulta($sql);
	...
	public function ventacabecera($idventa){
		$sql="SELECT v.idventa,v.idcliente,p.nombre as cliente,p.direccion,p.tipo_documento,p.num_documento,p.email,p.telefono,v.idusuario,u.nombre as usuario,v.codigotipo_comprobante,v.serie,v.correlativo,date(v.fecha_hora) as fecha,v.impuesto,v.op_gravadas,v.op_inafectas,v.op_exoneradas,v.op_gratuitas,v.isc,v.total_venta,v.idmoneda,v.idmotivo_doc,md.motivo,v.sustento,v.doc_relacionado, m.descripcion as descmoneda,( SELECT serie FROM venta where idventa=v.doc_relacionado) as serie_rela,( SELECT correlativo FROM venta where idventa=v.doc_relacionado) as correlativo_rela FROM venta v INNER JOIN persona p ON v.idcliente=p.idpersona INNER JOIN usuario u ON v.idusuario=u.idusuario INNER JOIN moneda m ON m.idmoneda=v.idmoneda INNER JOIN motivo_documento md ON md.idmotivo_documento=v.idmotivo_doc WHERE v.idventa='$idventa'";
		return ejecutarConsulta($sql);
	}
	...
	public function ventadetalle($idventa){
		$sql="SELECT a.nombre as articulo,a.unidad_medida,a.descripcion_otros,a.afectacion,dv.cantidad,dv.precio_venta,dv.descuento,dv.correccion_descripcion,(dv.cantidad*dv.precio_venta-dv.descuento) as subtotal FROM notacredito dv INNER JOIN articulo a ON dv.idarticulo=a.idarticulo WHERE dv.idventa='$idventa'";
		return ejecutarConsulta($sql);  
  }
