...
Class ticket
{
    ...
    $saber = "SELECT serie,correlativo FROM venta WHERE codigotipo_comprobante='12'";
        $saberExiste = ejecutarConsultaSimpleFila($saber);
        if($saberExiste["serie"] == null and $saberExiste["correlativo"] == null){
          $serie='N001'; 
          $correlativo='00000001';
        }else{
          $sqlmaxserie="SELECT max(serie) as maxSerie FROM venta WHERE codigotipo_comprobante='12' ";
          $maxserie = ejecutarConsultaSimpleFila($sqlmaxserie);
          $serie= $maxserie["maxSerie"];
          $ultimoCorrelativo="SELECT max(correlativo) as ultimocorrelativo,serie,correlativo FROM venta WHERE codigotipo_comprobante='12'  and serie='$serie'";
     ...
     $fecha_todo = date('Y-m-d H:i:s');
		...
		$sql="INSERT INTO venta (idcliente,idusuario,codigotipo_comprobante,serie,correlativo,fecha_hora,impuesto,op_gravadas,op_inafectas,op_exoneradas,op_gratuitas,isc,total_descuentos,total_igv,total_venta,leyenda,estado,idmoneda,idmotivo_doc,codigotipo_pago) VALUES ('$idcliente','$idusuario','$codigotipo_comprobante','$serie','$correlativo','$fecha_todo','$impuesto','$op_gravadas','$op_inafectas','$op_exoneradas','$op_gratuitas','$isc','$total_descuentos','$total_igv','$total_venta','$leyenda','Aceptado','$idmoneda',null,1)";
	...
	public function selectTipoComprobante(){
		$sql="SELECT * from tipo_comprobante WHERE codigotipo_comprobante = 12";
		return ejecutarConsulta($sql);
	}
