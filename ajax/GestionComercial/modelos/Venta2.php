...
Class Venta2
{
  ...
  public function insertar($idcliente,$idusuario,$codigotipo_comprobante,$codigotipo_pago,$fecha_hora,$fecha_ven,$impuesto,$op_gravadas,$op_inafectas,$op_exoneradas,$op_gratuitas,$isc,$total_descuentos,$total_igv,$total_venta,$leyenda,$idmoneda,$idarticulo,$cantidad,$precio_venta,$descuento,$serieArticulo){
    ...
    $sql="INSERT INTO venta (idcliente,idusuario,codigotipo_comprobante,serie,correlativo,fecha_hora,fecha_ven,impuesto,op_gravadas,op_inafectas,op_exoneradas,op_gratuitas,isc,total_descuentos,total_igv,total_venta,leyenda,estado,idmoneda,idmotivo_doc,codigotipo_pago) VALUES ('$idcliente','$idusuario','$codigotipo_comprobante','$serie','$correlativo','$fecha_todo','$fecha_ven','$impuesto','$op_gravadas','$op_inafectas','$op_exoneradas','$op_gratuitas','$isc','$total_descuentos','$total_igv','$total_venta','$leyenda','Aceptado','$idmoneda',null, '$codigotipo_pago')";
  ...
  public function ventacabecera($idventa){
    $sql="SELECT v.idventa,v.idcliente,p.nombre as cliente,p.direccion,p.tipo_documento,p.num_documento,p.email,p.telefono,v.idusuario,u.nombre as usuario,v.codigotipo_comprobante,v.serie,v.correlativo,date(v.fecha_hora) as fecha,v.fecha_hora as fechaCompleta,v.fecha_ven,v.impuesto,v.op_gravadas,v.op_inafectas,v.op_exoneradas,v.op_gratuitas,v.isc,v.total_venta,v.total_igv,v.idmoneda,m.descripcion as descmoneda, tp.descripcion_tipo_pago as codigo_pago FROM venta v 
  ...
  public function ventadetalle($idventa){
    $sql="SELECT a.nombre as articulo,a.unidad_medida,a.descripcion_otros,a.afectacion,d.cantidad,d.precio_venta,d.descuento,d.serie,(d.cantidad*d.precio_venta-d.descuento) as subtotal, a.codigo FROM detalle_venta d INNER JOIN articulo a ON d.idarticulo=a.idarticulo WHERE d.idventa='$idventa'";
    return ejecutarConsulta($sql);
  }
