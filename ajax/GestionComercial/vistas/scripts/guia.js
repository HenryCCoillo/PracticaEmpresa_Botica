...
function listarArticulos()
{
	tabla=$('#tblarticulos').dataTable(
	{
		...
		"ajax":
				{
					url: '../ajax/guia.php?op=listarArticulosVenta',
					type : "get",
					dataType : "json",
					error: function(e){
						console.log(e.responseText);
...
{
	$.post("../ajax/guia.php?op=mostrar",{idventa : idventa}, function(data, status)
	{
		data = JSON.parse(data);
		// mostrar cliente
 	...
 	$.post("../ajax/guia.php?op=listarDetalle&id="+idventa,function(r){
	        $("#detalles").html(r);
	});
...
function anular(idventa)
{
	bootbox.confirm("¿Está Seguro de anular la venta?", function(result){
		if(result)
        {
        	$.post("../ajax/guia.php?op=anular", {idventa : idventa}, function(e){
        		bootbox.alert(e);
	            tabla.ajax.reload();
        	});
        }
