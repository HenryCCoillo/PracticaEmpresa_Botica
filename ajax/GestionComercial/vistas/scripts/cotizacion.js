...
{
	$.post("../ajax/cotizacion.php?op=mostrar",{idventa : idventa}, function(data, status)
	{
 	...
 	$.post("../ajax/cotizacion.php?op=listarDetalle&id="+idventa,function(r){
	        $("#detalles").html(r);
	});
}
...
function anular(idventa)
{
	bootbox.confirm("¿Está Seguro de anular la cotización?", function(result){
		if(result)
        {
        	$.post("../ajax/cotizacion.php?op=anular", {idventa : idventa}, function(e){
        		bootbox.alert(e);
	            tabla.ajax.reload();
        	});
        }
