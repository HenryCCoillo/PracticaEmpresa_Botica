...
switch ($_GET["op"]){
 		...
 		while ($reg=$rspta->fetch_object()){
 			$data[]=array(
 				"0"=>($reg->condicion)?'<button class="btn btn-warning" onclick="mostrar('.$reg->idusuario.')"><i class="fa fa-pencil"></i></button> '.
					 ...
					 '<button class="btn btn-primary" onclick="eliminar('.$reg->idusuario.')"><i class="fa fa-trash"></i></button>':
	...
	case 'eliminar':
		...
		echo $rspta ? "Usuario eliminado" : "Persona no se pudo eliminar";
		break;
	break;
	...
	case 'verificar':
		...
		if(isset($fetch))
			...
			while($per=$marcados->fetch_object())
			{
				...
				in_array(8,$valores) ? $_SESSION['administracion']=1 : $_SESSION['administracion']=0;
